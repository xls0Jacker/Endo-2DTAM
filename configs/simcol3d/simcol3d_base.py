...
tracking_iters = 15 if down_scale == 2 else 5
mapping_iters = 15 if down_scale == 2 else 6
ba_iters = 200
ba_every = 100
...
config = dict(
    ...
    distance_current_frame_prob=0.1 if down_scale == 2 else 0.95, # Probability of choosing the current frame in mapping optimization
    ...
    data=dict(
        basedir="./data/SimCol3D",
        gradslam_data_cfg="./configs/data/simcol3d.yaml",
        sequence=scene_name,
        desired_image_height=475//down_scale,
        desired_image_width=475//down_scale,
        start=0,
        end=-1,
        stride=1,
        num_frames=-1,
